...
grammar.p_values <- grammar.anovas %>%
  ...
  mutate(linear_quadratic_p = linear_quadratic_p[[1]],
         quadratic_linear.quadratic_p = quadratic_linear.quadratic_p[[1]],
         linear.quadratic_quadratic.age_p = linear.quadratic_quadratic.age_p[[1]],
         quadratic.age_quadratic.age.noint_p = quadratic.age_quadratic.age.noint_p[[1]])
```

r-squared values of each model
```{r}
grammar.models %>% select(-linear, -quadratic, -linear.quadratic, -quadratic.age, -quadratic.age.noint)
```

p-values of each successive pair of models
```{r}
...
#  return(coef(lm(pos.prop ~ I((vocab.mean*100)^2) + I(vocab.mean*100) * age + 0))["I(vocab.mean * 100):age"])
  return(coef(lm(pos.prop ~ I((vocab.mean*100)^2) * age + 0))["I((vocab.mean * 100)^2):age"])
  }

prop.mod.se.fun <- function(pos.prop, vocab.mean, age) {
  return(summary(lm(pos.prop ~ I((vocab.mean*100)^2) * age + 0))$coefficients["I((vocab.mean * 100)^2):age", "Std. Error"])
  }

...
summarise.language.data <- function(lang.data,lang) {
  ...
  d.complexity <- lang.data %>%
    ...
    group_by(age,id) %>%
    summarise(complexity = sum(value == "complex", na.rm=TRUE) / length(value))
  
  d.wordform <- lang.data %>%
    filter(type == "word_form") %>%
    group_by(age,id) %>%
    summarise(wordform = sum(value == "produces", na.rm=TRUE) / length(value))
...
summary.data <- rbind_list(summary.english,summary.spanish,
  ...
  filter(age > 12 & age < 32) %>%
  mutate(age.group = cut(age, breaks = c(12, 16, 20, 24, 28, 32)),
...
ms %>% select(-s.vocab, -s.age) %>% group_by(language, age.group) %>% summarise(n = n())
```

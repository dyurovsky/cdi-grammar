...
```{r,message=FALSE}
aggregate.wdbank <- function(table, items, language, form) {
  
  instrument.items <- items %>% filter(language == language, 
                                       form == form) %>%
    select(item, type, category, lexical_category)
  
  instrument.data <- as.data.frame(table) %>%
  ...
  d.vocab <- d %>%
    filter(type == "word") %>%
    group_by(id) %>%
    summarise(vocab = sum(value == "produces", 
                          na.rm=TRUE))
  
  d.syntax <- d %>%
    filter(type == "complexity") %>%
    group_by(id) %>%
    summarise(complexity = mean(value == "complex", 
                                na.rm=TRUE))
  
  d.wordform <- d %>%
    filter(type %in% c("word_form")) %>%
    group_by(id) %>%
    summarise(wordform = mean(value == "produces", 
                              na.rm=TRUE))
...
```{r}
d.english <- aggregate.wdbank(table=english.ws.table, 
                              items=items, 
                              language="English", 
                              form="WS")
d.norwegian <- aggregate.wdbank(table=norwegian.ws.table, 
                                items=items, 
                                language="Norwegian", 
                                form="WS")
d.danish <- aggregate.wdbank(table=danish.ws.table, 
                                items=items, 
                                language="Danish", 
                                form="WS")
```
...
```{r}
d.english$lang <- "English"
d.norwegian$lang <- "Norwegian"
d.danish$lang <- "Danish"

d <- rbind_list(d.english,d.norwegian,d.danish)
d <- left_join(d, child.data)
d$age.grp <- cut(d$age, breaks=c(0,16,20,24,28,32,36))
```
...
```{r}
quartz()
qplot(vocab, complexity, col=age.grp,
      facets=~lang,
      data=d) + 

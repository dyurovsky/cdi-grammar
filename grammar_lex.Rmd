...
Fit grammar models and get their AICs and coefficients.
```{r}
grammar.models <- ms %>%
  filter(!is.na(sum)) %>%
...
Fit composition models and predict data.
```{r}
...
Compute coefficients of vocab composition models.
```{r}
...
Plot vocab composition coefficients.
```{r}
#quartz(width=6, height=4)
ggplot(comp.models, 

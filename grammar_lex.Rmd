...
english.ws.items <- items %>% filter(language == 'English', form == 'WS') %>%
  select(item, type, category, lexical_category)

english.ws.data <- as.data.frame(english.ws.table) %>%
  rename(id = basetable_ptr_id) %>% # Rename the id
  gather(item,value,-id) %>% # Arrange in longform
  mutate(item = str_replace(item, "item_", "")) # Strip off item_ from items

english.ws <- left_join(english.ws.data, english.ws.items)

english.ws.vocab.scores <- english.ws %>%
  filter(type == 'word') %>%
  group_by(id) %>%

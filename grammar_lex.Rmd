...
#   mutate(linear_coef = summary(linear)$coefficients["", "p"],
#          quadratic_rsq = summary(quadratic)$adj.r.squared)
# 
# grammar.anovas <- grammar.models %>%
#   do(linear_quadratic_anova = anova(.$linear,
...
for (lang in c("English", "Spanish", "Norwegian", "Danish")) {
  for (meas in c("Word Form", "Complexity")) {
    model <- glm(cbind(sum,diff) ~ vocab.mean + age.group,
                 ...
                 family="binomial")
    predict <- inv.logit(predict.lm(model,
...
glm.predicted.data$language <- factor(glm.predicted.data$language,
...
lang.interaction.terms <- function(grammar.by.item) {
  ...
  interaction.terms <- grammar.by.item %>%
    group_by(item) %>%
  ...
  interaction.terms$coded.type = factor(interaction.terms$coded.type,
                                        levels = c("word_form",
                                                   "morphology",
                                                   "syntax"),
                                        labels = c("Word Form",
...
Function for computing and plotting interaction terms by item for a language.
```{r}
interaction.plot <- function(lang.interaction.terms, language) {
  plt <- ggplot(lang.interaction.terms,
    ...
    annotate("text", x = length(lang.interaction.terms$item)/2,
             y = min(lang.interaction.terms$coef),

...
summarise.language.data <- function(lang.data,lang) {
  ...
  d.complexity <- lang.data %>%
    ...
    select(-all.na) # Deals with ifelse 
...
```{r}
linear <- formula(cbind(sum,diff) ~ I(vocab.mean*100)*measure)
linear.age <- formula(cbind(sum,diff) ~ I(vocab.mean*100)*measure*age)
grammar.formulas <- c(linear, linear.age)

grammar.models <- ms %>%
  group_by(language) %>%
  do(linear = glm(grammar.formulas[[1]], ., family="binomial"),
     linear.age = glm(grammar.formulas[[2]], ., family="binomial")) %>%
  mutate(linear_coef = summary(linear)$["", "p"],
         quadratic_rsq = summary(quadratic)$adj.r.squared)
...
#glm.predicted.data <- ms
glm.predicted.data$predicted <- NA

for (lang in c("English", "Spanish", "Norwegian", "Danish")) {
    model <- glm(cbind(sum,diff) ~ I(vocab.mean*100) * age.group * measure,
                         data = filter(glm.predicted.data, language==lang), family="binomial")
#    model <- glm(score ~ I(vocab.mean*100) * age.group * measure,
#                         data = filter(glm.predicted.data, language==lang), family="binomial")
    predict <- inv.logit(predict.lm(model, glm.predicted.data[glm.predicted.data$language==lang,]))
   # print(cor.test(predict, glm.predicted.data[glm.predicted.data$language==lang,]$score))
    glm.predicted.data[glm.predicted.data$language==lang,]$predicted <- predict
}
...
glm.predicted.data$language <- factor(glm.predicted.data$language, levels = c("Norwegian", "English", "Danish", "Spanish"))
...
glm.predicted.data$language <- factor(glm.predicted.data$language, levels = c("Norwegian", "English", "Danish", "Spanish"))
quartz(width=6,height=6)
ggplot(glm.predicted.data, aes(x = age, y = mean,
                               colour = measure)) +
#                           colour = age.group, fill = age.group,
#                           label = age.group)) + 
#  geom_jitter(alpha=.3, size=.75) +
...
models <- ms %>%
  filter(!is.na(sum)) %>%
  group_by(language, measure) %>%
  do(model.age = glm(cbind(sum,diff) ~ age, data = ., family="binomial"),
     model.vocab = glm(cbind(sum,diff) ~ vocab.mean, data = ., family="binomial"),
     model.vocab.age = glm(cbind(sum,diff) ~ vocab.mean + age, data = ., family="binomial")) %>%
  mutate(AIC.age = AIC(model.age),
         AIC.vocab = AIC(model.vocab),
         AIC.vocab.age = AIC(model.vocab.age))
#summary(filter(models, language=="Danish", measure=="Complexity")$model[[1]])
```
...
cor(eng.data.cmp$mean,eng.data.cmp$predicted2)-cor(eng.data.cmp$mean,eng.data.cmp$predicted1)
...
test.binomial <- with(filter(prop.noun.verb.diff),
                      glm(cbind(pos.size,pos.unknown) ~vocab.mean*age*pos + 0,family = "binomial"))

...
english.lm <- lm(score  ~ I((vocab.mean*100)^2)*measure*age.group, 
                 data=filter(ms,language=="English"))
spanish.lm <- lm(score  ~ I((vocab.mean*100)^2)*measure*age.group, 
                 data=filter(ms,language=="Spanish"))
norwegian.lm <- lm(score  ~ I((vocab.mean*100)^2)*measure*age.group, 
                   data=filter(ms,language=="Norwegian"))
danish.lm <- lm(score  ~ I((vocab.mean*100)^2)*measure*age.group, 
                data=filter(ms,language=="Danish"))
# No significant interaction term for Danish?

predicted.data <- as.data.frame(ms)

predicted.data$predicted <- NA

predicted.data[predicted.data$language=="English",]$predicted <- 
  predict.lm(english.lm,predicted.data[predicted.data$language=="English",])
predicted.data[predicted.data$language=="Spanish",]$predicted <- 
  predict.lm(spanish.lm,predicted.data[predicted.data$language=="Spanish",])
predicted.data[predicted.data$language=="Norwegian",]$predicted <- 
  predict.lm(norwegian.lm,predicted.data[predicted.data$language=="Norwegian",])
predicted.data[predicted.data$language=="Danish",]$predicted <- 
  predict.lm(danish.lm,predicted.data[predicted.data$language=="Danish",])
```
...
# english.syntax <- english.grammar.by.item %>%
#   filter(type=="complexity") %>%
#   ungroup() %>%
#   spread(item, value) %>%
#   select(-age, -vocab.sum, -vocab.mean, -type, -definition, -child.id, -source.id, -sex, -mom_ed, -birth_order)
# 
# complex.cors <- cor(as.matrix(as.data.frame(eng.syntax.vocab)[,11:ncol(eng.syntax.vocab)]),use="complete") %>%
#  as.data.frame
# 
# complex.cors <- cor(tmp[])
...
```{r}
means.comp <- left_join(filter(summary.vocab.comp, lexical_category != "Other"),
                        lang.vocab.comp) %>%
  mutate(lexical_category = factor(lexical_category,
                                   levels = c("Nouns", "Predicates", 
                                              "Function Words", "Other")), 
...
means.comp <- left_join(ms, means.comp) %>%
...
plot.vocab.comp.prediction <- function(category){
  p <- ggplot(filter(means.comp, lexical_category==category),
...
```{r, fig.width=10, fig.height=8}
ggplot(filter(means.comp, lexical_category != "Other"),
       aes(x=vocab.mean, y=cdi.prop, colour=lexical_category, 
           shape = lexical_category, fill = lexical_category,
           label=lexical_category)) +
  geom_jitter(size = .75, alpha = 0.25) +
  facet_wrap(~ language) +
#  geom_hline(data=lang.vocab.comp,aes(yintercept=prop.per.cat),
#             linetype="dashed", color="grey") + #baselines for each language
  geom_smooth(aes(group=lexical_category), method='loess', span=0.5) +

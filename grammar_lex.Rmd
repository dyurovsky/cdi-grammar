...
% Mika Braginsky, Dan Yurovsky, Virginia Marchman, and Mike Frank
...
quadratic.age <- formula(score ~ I((vocab.mean*100)^2) * age)
quadratic.age.noint <- formula(score ~ I((vocab.mean*100)^2) * age + 0)
grammar.formulas <- c(linear, quadratic, linear.quadratic, quadratic.age, quadratic.age.noint)
...
grammar.models <- ms %>%
  ...
  do(linear = lm(grammar.formulas[[1]], .),
     ...
     quadratic.age = lm(grammar.formulas[[4]], .),
     quadratic.age.noint = lm(grammar.formulas[[5]], .)) %>%
  mutate(linear_rsq = summary(linear)$adj.r.squared,
         ...
         quadratic.age_rsq = summary(quadratic.age)$adj.r.squared,
         quadratic.age.noint_rsq = summary(quadratic.age.noint)$adj.r.squared)

grammar.anovas <- grammar.models %>%
  do(linear_quadratic_anova = anova(.$linear, .$quadratic),
     quadratic_linear.quadratic_anova = anova(.$quadratic, .$linear.quadratic),
     linear.quadratic_quadratic.age_anova = anova(.$linear.quadratic, .$quadratic.age),
     quadratic.age_quadratic.age.noint_anova = anova(.$quadratic.age, .$quadratic.age.noint))
...
grammar.p_values <- grammar.anovas %>%
  do(linear_quadratic_p = .$linear_quadratic_anova$`Pr(>F)`[[2]],
     ...
     linear.quadratic_quadratic.age_p = .$linear.quadratic_quadratic.age_anova$`Pr(>F)`[[2]],
     quadratic.age_quadratic.age.noint_p = .$quadratic.age_quadratic.age.noint_anova$`Pr(>F)`[[2]]) %>%
  mutate(linear_quadratic_p = log10(linear_quadratic_p[[1]]),
         quadratic_linear.quadratic_p = log10(quadratic_linear.quadratic_p[[1]]),
         linear.quadratic_quadratic.age_p = log10(linear.quadratic_quadratic.age_p[[1]]),
         quadratic.age_quadratic.age.noint_p = log10(quadratic.age_quadratic.age.noint_p[[1]]))
```

Show R-squared values of some models.
```{r,results = 'asis'}
display.models <- grammar.models %>% select(language, measure, linear.quadratic_rsq, quadratic.age_rsq, quadratic.age.noint_rsq)
...
display.p_values <- grammar.p_values %>% select(language, measure, linear.quadratic_quadratic.age_p, quadratic.age_quadratic.age.noint_p)
...
for (lang in c("English", "Spanish", "Norwegian", "Danish")) {
  for (meas in c("Complexity", "Word Form")) {
    model <- lm(score ~ I((vocab.mean*100)^2) * age.group,
                data=filter(ms,language==lang, measure==meas))
    pinned.model <- lm(score ~ I((vocab.mean*100)^2) * age.group - age.group + 0,
                data=filter(ms,language==lang, measure==meas))
...
i.terms.function <- function(data,x) {
  return(coef(lm(value ~ I((vocab.mean*100)^2) * age + 0,
              data=filter(data,definition==x)))["I((vocab.mean * 100)^2):age"])
#  return(summary(lm(value ~ I((vocab.mean*100)^2) * age + 0, 
#                    data=filter(data,definition==x))$coefficients[3,3])
  }
...
#  return(coef(lm(pos.prop ~ I((vocab.mean*100)^2) + I(vocab.mean*100) * age + 0))["I(vocab.mean * 100):age"])
  return(coef(lm(pos.prop ~ I(vocab.mean*100) * age + 0))["I(vocab.mean * 100):age"])
  }

prop.mod.se.fun <- function(pos.prop, vocab.mean, age) {
  return(summary(lm(pos.prop ~ I(vocab.mean*100) * age + 0))$coefficients["I(vocab.mean * 100):age", "Std. Error"])
  }
